...
userEmail.addEventListener("focusout", (e) => {
  ...
  if (userEmailVal === "") {
    ...
    btnFail();
  } else if (!emailTest.test(userEmailVal)) {
    // 이메일 값이 공백은 아니지만 이메일 형식에 맞지 않을 때
...
if (userPasswordChk) {
  userPasswordChk.addEventListener("focusout", (e) => {
    // 비밀번호 확인 input에서 focusout 될 경우 실행
    const userPasswordChkVal = e.target.value.split(" ").join(""); // split(" ").join("")으로 value 띄어쓰기 삭제

    if (userPasswordChkVal === "") {
      // 비밀번호 값이 공백일 때
      e.target.className = "fail";
      e.target.nextElementSibling.textContent = "비밀번호 일치하지 않습니다.";
      e.target.nextElementSibling.classList.add("on");
      btnFail();
    } else if (userPassword.value !== userPasswordChkVal) {
      // 비밀번호 값이 일치 하지 않을 때
      e.target.className = "fail";
      e.target.nextElementSibling.textContent = "비밀번호 일치하지 않습니다.";
      e.target.nextElementSibling.classList.add("on");
...
passwordBtn.addEventListener("click", (e) => {
  ...
  if (userPassword.getAttribute("type") !== "password") {
    // 비밀번호 input type가 password가 아닐경우
...
if (passwordBtnChk) {
  passwordBtnChk.addEventListener("click", (e) => {
    // 비밀번호 확인 온오프 버튼 기능
    let passwordBtnImg = passwordBtnChk.firstChild; // 비밀번호 확인 온오프 이미지

    if (userPasswordChk.getAttribute("type") !== "password") {
      // 비밀번호 확인 input type가 password가 아닐경우
      userPasswordChk.setAttribute("type", "password"); // input type를 password로 변경
      passwordBtnImg.setAttribute("src", "images/i-password-off.png"); // 온오프 이미지를 오프로 변경
    } else {
      // 비밀번호 확인 input type가 password
...
if (userName && userPasswordChk && passwordBtnChk) {
  function btnAble() {
    ...
    if (emailVal && nameVal && passwordVal && passwordChkVal) {
      // 이메일도 정상적으로 입력하고 비밀번호도 8자리 이상일경우
      submitBtn.disabled = false;
      submitBtn.classList.add("on");
    } else {
      // 이메일 비밀번호 둘다 조건에 맞지 않을 경우
      btnFail();
    }
  }
